...
if __name__ == '__main__':
    ...
    opt = torch.optim.Adam(list(model_unet.parameters()) + list(context_embedder.parameters()), lr = 1e-3)

    # context, context_shape = get_image_condition('./context_imgs/cond_diff_context_6.jpg', condition_channels, device)
    ...
    for i in range(steps):
        ...
        context = context_embedder(labels).unsqueeze(1) # batch_size x condition_channels -> batch_size x 1 x condition_channels
    ...
    torch.save({'unet': model_unet.state_dict(), 
                'embedder': context_embedder.state_dict()}, 
