...
from conditioning import get_image_condition
...
if __name__ == '__main__':
    ...
    context, context_shape = get_image_condition('./context_imgs/cond_diff_context_6.jpg', condition_channels = 128, device = device)
    ...
    ##########set UNet model
    unet_cosine = UNet(in_channels=1).to(device)
    # unet_linear = UNet(in_channels=1).to(device)
    unet_cosine.load_state_dict(torch.load('results/model_UNet_cosine_1000timesteps_256batch_20000steps.pt', map_location=device))
